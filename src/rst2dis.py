# coding=utf-8
# Copyright 2020 Amir Zeldes
# Copy from https://github.com/amir-zeldes/gum/blob/e44a7862c59584a9ed6ac122c5018bd67c3a4c60/_build/utils/rst2dis.py
...
class Unit:
	...
	def __repr__(self):
		return str(self.id) + "[" + str(self.left) + "-" + str(
			self.right) + "]" + " (" + self.type + ")" + ": " + self.text
...
def rst2dis(rs3_string, remove_unary=True, binarize=True, ignore_heading=True):
	"""Convert an RS3 string to a .dis string.
	
	:param rs3_string: RST tree as a string in `.rs3` format
	:param remove_unary: Whether to remove unary branches (default True)
	:param binarize: Whether to binarize n-ary branches (default True)
	:param ignore_heading: Whether to ignore heading segments (default True)

	---

	### Note
	By some conventions, headings of a text are not considered part of the
	discourse structure and are thus sometimes annotated as a disjunct segment.
	If `ignore_heading` is set to True, segments with `id='1'` and without a
	parent, will be excluded from the resulting `.dis` tree. 
	"""
	def collapse_segment_line(m: re.Match) -> str:
		return "".join([
			m.group(1).replace("\n", ""),
			m.group(2).replace("\n", ""),
			m.group(3).replace("\n", "")
	...
	def make_heading_unit(m: re.Match) -> Unit:
		nonlocal i, line
		if m.group(1) == '1':
			if "parent" in m.group(2):
				raise IOError(
					"Invalid header-line " + str(i) + ":\n" + line
					+ f"\n\tMatched: {str(m.groups())}"
				)
		u = Unit()
		u.id = int(m.group(1))
		u.text = m.group(3)
	...
	multi_ln_segment = r"(?sm:(<segment[^>]*?>)(.*?)(</segment>))"
	rs3_string = re.sub(
		multi_ln_segment, collapse_segment_line, rs3_string
	)
	...
	for i, line in enumerate(lines):
		if "<rel" in line:
			...
			if m is not None:
				...
				# Heading segment without a parent
				heading_unit = make_heading_unit(m)
				if heading_unit.id > max_right:
					max_right = heading_unit.id
				units[heading_unit.id] = heading_unit
			else:
				raise IOError(
					"Invalid input at line " + str(i) + ": " + line
				)
		elif "<group" in line:
	...
	# add heading as child to root
	try:
		heading_unit
	except NameError:
		heading_unit = None
	if ignore_heading and heading_unit is not None:
		heading_unit.parent = root.id
		units[heading_unit.id] = heading_unit
		root_descendants = descendants[root.id]
		root_descendants.add(heading_unit.id)
		descendants[root.id] = root_descendants
		descendants[heading_unit.id] = set([])
		root.children.insert(0, heading_unit)
	...
	# insert heading node as first child of root
	if ignore_heading and heading_unit is not None:
		root_children = root.children
		root.children = [
			c for c in root.children if c.id == heading_unit.id 
		] + [
			c for c in root_children if c.id != heading_unit.id
		]
